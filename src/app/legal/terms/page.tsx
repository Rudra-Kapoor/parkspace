...
export const metadata: Metadata = {
  ...
  description:
    'The draft terms that govern the use of ParkSpace. An unreviewed draft that has not been ' +
    'settled by a lawyer and must not be relied upon.',
...
async function loadDocument(): Promise<string | null> {
  try {
    ...
    // The file carries its own numbered title. The page supplies a cleaner one,
    // so the first heading is dropped to avoid two competing titles.
    ...
    <>
      <header>
        ...
        <p className="mt-3 text-[var(--text-muted)]">
          The general terms for using ParkSpace. While the separate general terms are being
          drafted, the driver agreement below carries them. If you list a space, the{' '}
          <Link href="/legal/host-terms" className="font-medium text-[var(--accent-text)] underline underline-offset-2">
            host agreement
          </Link>{' '}
          applies to you in addition.
        </p>
      </header>
      ...
      <Alert tone="warning" title="Unreviewed draft. Do not rely on this document." className="mt-6">
        ...
        accountant. It is published here because the product is being built in the open, not
        because it is finished. It is not legal advice, it binds nobody, and no part of it
        should be relied upon. Passages marked REVIEW REQUIRED are known open questions that
        must be answered by qualified counsel before this page can be treated as real.
      </Alert>
